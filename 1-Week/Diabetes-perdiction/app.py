...
def predict_diabetes(data, best_model_only=False):
...
def app():
    ...
    if input_method == "Upload CSV File":
        ...
        if uploaded_file is not None:
            try:
                ...
                for _, row in df.iterrows():
                    pred = predict_diabetes(row.values.tolist())
        ...
        if st.button("Predict"):
            ...
            # Make prediction
            predictions = predict_diabetes(data_values, best_model_only=(pred_type == "Use Best Model Only"))
